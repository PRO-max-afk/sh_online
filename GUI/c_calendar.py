...
class Calendar(QDialog):
    def __init__(self, main_window):
        ...
        # گرفتن تاریخ میلادی از سرور NTP
        self.current_date = self.get_gregorian_from_ntp()
    ...
    def get_gregorian_from_ntp(self):
        try:
            client = ntplib.NTPClient()
            response = client.request("pool.ntp.org", version=3, timeout=3)
            return datetime.fromtimestamp(response.tx_time, tz=timezone.utc).astimezone()
