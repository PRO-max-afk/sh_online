...
class DataLoaderThread(QThread):
        ...
        try:
            ...
            cursor.execute('''
                SELECT product_name, barcode,category,sub_category,buy_date,buy_price, sell_price,
                           ...
                           new_price, discount_percent, big_price,big_quantity,big_sub,big_sub_display,sale_unit, total
                FROM inventories
            ...
            for product in products:
                name, barcode,category, sub_category,buy_date,buy_price, sale_price,big_category,quantity, exp_date, image_path, store_name, new_price, discount_percent, big_price,big_quantity,big_sub,big_sub_display, sale_unit,total= product
    ...
    def store_in_local_db(self, product_list):
        ...
        for product in product_list:
            if all(key in product for key in ["barcode", "name", "category","sub_category","buy_date","buy_price", "sale_price","big_category","store_name", "new_price", "discount_percent", "big_price","big_quantity" ,"big_sub","big_sub_display","total","sale_unit","quantity", "expire_date", "image_path", "user_id"]):
                    ...
                    cursor.execute('''
                        INSERT INTO products (barcode, name, category,sub_category,buy_date,
                                   buy_price, sale_price,big_category,store_name, new_price, discount_percent, big_price,big_quantity,big_sub,big_sub_display,
                                    total, sale_unit,quantity, expire_date, image_path, user_id)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?,?,?,?,?,?)
                    ''', (
                        product["barcode"],
                        product["name"],
